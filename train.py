...
def main(epochs, batch_size, learning_rate, save_freq, data_dir):
    ...
    train_loader = Data.DataLoader(dataset=datasource, batch_size=batch_size, shuffle=True, pin_memory=True)

    # load model
    posenet = PoseNet().to(device)
    # # load to continue
    save_filename = 'epoch_{}.pth'.format('181'.zfill(5))
    ...
    # loss function
    criterion = PoseLoss(0.3, 0.3, 1., 900, 900, 900)
